...
class MatrixFactory:
    def __init__(self, gate: Gate, identities_flag: int, sparse: bool = False) -> None:
        self.gate: Gate = gate
    ...
    def apply_identities_and_controls(
        ...
        result = sparse_identity(shape, dtype=np.complex128, format="csr")
        result = result.tolil()
    ...
    def wrap_in_identities(
        cls, matrix: NDArray[np.complex128], indices: list[int], sizes: list[int], sparsity_flag: bool = False
    ) -> NDArray[np.complex128]:
        matrix = csr_matrix(matrix)
        indices.sort()
        ...
        i = 0
        result = sparse_identity(sizes[i], dtype=np.complex128, format="csr")
        while i < len(sizes):
            if i == indices[0]:
                result = matrix if i == 0 else sparse_kron(result, matrix)
            elif (i < indices[0] and i != 0) or i > indices[-1]:
                result = sparse_kron(result, sparse_identity(sizes[i], dtype=np.complex128, format="csr"))
